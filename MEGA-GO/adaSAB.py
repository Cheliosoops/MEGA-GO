...
class MAB(torch.nn.Module):
    ...
    def reset_parameters(self):
        ...
        # self.threshold.reset_parameters()
        pass
    ...
    def forward(
        ...
        if mask is not None:
            ...
            Mask_threshold = self.sigmoid(self.threshold)
            if Weather == "hot":
            
                attention_score = torch.where(attention_score < Mask_threshold, self.mask_threshold, attention_score)
            elif Weather == "cold":
                attention_score = torch.where(attention_score > Mask_threshold, self.mask_threshold, attention_score)
            else:
...
class PMA(torch.nn.Module):
    ...
    def __init__(self, channels: int, num_heads: int, num_seeds: int,
                 Conv: Optional[Type] = None, layer_norm: bool = True):
        super().__init__()
...
class GraphMultisetTransformer(torch.nn.Module):
    ...
    def forward(self, x: Tensor, batch: Tensor,
                edge_index: Optional[Tensor] = None, Weather: str = "") -> Tensor:
        """"""
        x = self.lin1(x)
        ...
        for i, (name, pool) in enumerate(zip(self.pool_sequences, self.pools)):
            ...
            batch_x = pool(batch_x, graph, mask,Weather)
            mask = None
